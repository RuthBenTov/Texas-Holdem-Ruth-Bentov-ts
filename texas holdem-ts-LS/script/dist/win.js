...
function getBestHandForEachPlayer(player) {
    ...
    arrayHandCards.forEach(function (c) { return (c.partOfSet = true); });
    return arrayHandCards;
}
var cardsArray = [
    new Card("2", "heart"),
...
function getHandName(cards) {
    ...
    switch (functionIndex) {
        ...
        case 4:
            return "Straight";
        case 3: {
            var cardNumber = getThreeOfKind(cards)[0].cardNumber;
        ...
        case 0: {
            var cardNumber = getHighCard(cards)[0].cardNumber;
...
function renderWinHand(player) {
    ...
    var cardsElements = player.allCards
        .map(function (c) {
        if (c.partOfSet) {
